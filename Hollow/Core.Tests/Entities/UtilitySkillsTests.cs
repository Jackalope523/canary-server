...
namespace Core.Tests.Entities
{
	public class ArbiterTests
	{
		...
		{
			// Act & Assert
			Arbiter.Try(true, new UnexpectedFailureException()); // No exception should be thrown
		}
		...
		{
			// Act & Assert
			Assert.Throws<UnexpectedFailureException>(() => Arbiter.Try(false, new UnexpectedFailureException()));
		}
		...
		{
			// Act & Assert
			Arbiter.Fail(false, new UnexpectedFailureException()); // No exception should be thrown
		}
		...
		{
			// Act & Assert
			Assert.Throws<UnexpectedFailureException>(() => Arbiter.Fail(true, new UnexpectedFailureException()));
		}
	}
	...
	public class PsijicTests
	{
		...
		[Fact]
		public void HasYet_TimeBeforeCurrentTime_ReturnsTrue()
		{
			// Arrange
		...
		[Fact]
		public void HasYet_TimeAfterCurrentTime_ReturnsFalse()
		{
			// Arrange
		...
		[Fact]
		public void HasAlready_TimeBeforeCurrentTime_ReturnsFalse()
		{
			// Arrange
			DateTimeOffset pastTime = DateTimeOffset.UtcNow.AddMinutes(-1);
		...
		[Fact]
		public void HasAlready_TimeAfterCurrentTime_ReturnsTrue()
		{
			// Arrange
			DateTimeOffset futureTime = DateTimeOffset.UtcNow.AddMinutes(1);
